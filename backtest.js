...
export default async function handler(req, res) {
  ...
  const cacheKey = `bt:v4:${strategy}:${index}:${expiry}:${dte}:${lots}:${strikePct}:${width}:${fromYear}:${toYear}:${slPct}:${tpPct}`;
  try {
    const cached = await kv.get(cacheKey);
    if (cached) {
  ...
  try {
    ...
    for (const expiryDay of expiryDays) {
      ...
      daily.push({ date: entryDay.date, expiryDate: expiryDay.date, pnl, entryPremium, exitPremium, spot: entrySpot, exitSpot, vix, dte: dteUsed });
    }
